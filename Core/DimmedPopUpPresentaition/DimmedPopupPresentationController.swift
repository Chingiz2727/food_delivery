...
final class DimmedPopupPresentationController: UIPresentationController {
  ...
  override var frameOfPresentedViewInContainerView: CGRect {
    ...
    let maxHeight = containerView.bounds.height - max(containerView.safeAreaInsets.top, 20) - 12
  ...
  override func dismissalTransitionDidEnd(_ completed: Bool) {
    ...
    if completed {
      dimmingView.removeFromSuperview()
      if let vc = presentedViewController as? DismissActionable {
        vc.actionAfterDismiss()
      }
    }
  }
