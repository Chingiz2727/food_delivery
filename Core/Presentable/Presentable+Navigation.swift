...
public extension Presentable {
  ...
  func embededInBackContainer(withTrailingViewInTitleModule: Presentable? = nil) -> Presentable? {
    guard let controllerToPresent = toPresent() else { return nil }
    let containerController = BackContainerViewController(
      rootViewController: controllerToPresent,
      trailingViewInTitleController: withTrailingViewInTitleModule?.toPresent()
    )
    return containerController
  }

  func embededInCardContainer() -> Presentable? {
    guard let controllerToPresent = toPresent() else { return nil }
    let containerController = CardContainerViewController(rootViewController: controllerToPresent)
    return containerController
  }
  ...
  func withBackDismissButton() -> Presentable? {
    guard let controllerToPresent = toPresent() else { return nil }
    return BackButtonContainerViewController(rootViewController: controllerToPresent)
  }
}
