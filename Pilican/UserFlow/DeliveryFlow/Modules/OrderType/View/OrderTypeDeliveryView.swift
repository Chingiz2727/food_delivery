...
class OrderTypeDeliveryView: UIView {
    let imageView: UIImageView = {
        let imageView = UIImageView()
        imageView.image = Images.deliveryType.image
        return imageView
    }()

    let titleLabel: UILabel = {
        let label = UILabel()
        label.text = "Доставка Pillikan"
    ...
    let subTitleLabel: UILabel = {
        let label = UILabel()
        label.text = "Доставка через 55 мин."
